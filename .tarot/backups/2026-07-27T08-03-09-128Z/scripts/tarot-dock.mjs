...
function drawHeader(restoreCursor = true) {
  ...
  const statusLines = [
    ...
    ...serviceLines,
    paint('35', `├${bar}┤`),
    `  ${paint(String(state.accentColor || accentColor), '⌘ LINK RELAYS')}`,
    `  ${paint('2', clip(links, width - 4))}`,
    `  ${paint('2', `COPY   type 1–${state.addresses.length} + Return → clipboard     ·     C / c → LAN relay`)}`,
    paint('35', `├${bar}┤`),
    `  ${paint(String(state.accentColor || accentColor), '✧ CONTROL')}${paint('2', '   awaken · start <service> · stop <service> · banish · restart <service>')}`,
    `  ${paint(String(state.accentColor || accentColor), '◌ OBSERVE')}${paint('2', `   scry <1–${state.services.length}> · logs <1–${state.services.length}> · view · help`)}`,
    paint('35', `└${bar}┘`),
  ];

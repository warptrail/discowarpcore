...
function setServicePort(service, port) {
  if (!service) return;
  service.port = port;
  service.env = { ...(service.env || {}) };
  if (service.id === 'backend') service.env.PORT = String(port);
  if (service.id === 'frontend') {
    const backend = services.get('backend');
    service.env.VITE_PORT = String(port);
    if (backend?.port) service.env.VITE_API_TARGET = `http://127.0.0.1:${backend.port}`;
    service.args = [...(service.args || [])];
    const portFlagIndex = service.args.findIndex((argument) => argument === '--port');
    if (portFlagIndex >= 0) service.args[portFlagIndex + 1] = String(port);
  }
}

function restorePersistedApplicationPorts() {
  if (!existsSync(statePath)) return;
  try {
    const state = JSON.parse(readFileSync(statePath, 'utf8'));
    const saved = new Map((state.services || []).map((service) => [service.id, Number(service.port)]));
    const backendPort = saved.get('backend');
    const frontendPort = saved.get('frontend');
    if (Number.isInteger(backendPort) && Number.isInteger(frontendPort) && frontendPort === backendPort + 1) {
      setServicePort(services.get('backend'), backendPort);
      setServicePort(services.get('frontend'), frontendPort);
    }
  } catch {}
}

async function allocateAdjacentApplicationPorts() {
  const backend = services.get('backend');
  const frontend = services.get('frontend');
  if (!backend || !frontend) return;
  if (frontend.port !== backend.port + 1) {
    throw new Error(`Canonical port policy requires frontend :${backend.port + 1} immediately after backend :${backend.port}.`);
  }
  setServicePort(backend, backend.port);
  setServicePort(frontend, frontend.port);
}

restorePersistedApplicationPorts();
...
let terminalWidth = Math.max(4, process.stdout.columns || 88);
let terminalRows = Math.max(24, process.stdout.rows || 24);
let warpFrame = 0;
...
async function control(parts) {
  ...
  if (verb === 'reflow') {
    setTerminalSize(argument, parts[2]);
    // Terminal has already rewrapped the old scroll-region drawing. Clearing
    // the visible viewport is the only reliable way to remove those fragments
    // before creating a fresh region at the new dimensions.
...
function drawHeader(restoreCursor = true, clearViewport = false) {
  if (!process.stdout.isTTY || !existsSync(statePath)) return;
  const state = JSON.parse(readFileSync(statePath, 'utf8'));
  // Never force a minimum or maximum Dock width. Terminal reports the actual
  // usable columns, and every visible row below is clipped to that same frame.
  // This prevents line wrapping from pushing purple rails out of alignment.
  const width = terminalWidth;
  const compact = width < fullDockMinColumns;
  const contentWidth = Math.max(0, width - 4);
  const bar = '─'.repeat(Math.max(0, width - 2));
  const dockLine = (value, color = '2') => `  ${paint(color, clip(value, contentWidth))}`;
  ...
  const serviceLines = state.services.map((service, index) => {
    ...
    return dockLine(`${marker} ${name} ${status.padEnd(7)} OWNER / ${owner}`, color);
  });
  ...
  const title = `TAROT / ${state.projectName.toUpperCase()}  ·  ${rotatedSigil}  ·  ${serviceSummary}  ·  ${online}`;
  const statusLines = [
    paint('35', `┌${bar}┐`),
    `│ ${paint(String(state.accentColor || accentColor), clip(title, Math.max(0, width - 3)))}`,
    dockLine('✦ STATUS DECK  ·  scry slot / service     port      state    ownership'),
    paint('35', `├${bar}┤`),
    ...serviceLines,
    paint('35', `├${bar}┤`),
    dockLine('⌘ LINK RELAYS', String(state.accentColor || accentColor)),
    dockLine(links),
    dockLine(`COPY   type 1–${state.addresses.length} + Return → clipboard     ·     C / c → LAN relay`),
    paint('35', `├${bar}┤`),
    dockLine('✧ CONTROL   awaken · start <service> · stop <service> · banish · restart <service>', String(state.accentColor || accentColor)),
    dockLine(`◌ OBSERVE   scry <1–${state.services.length}> · logs <1–${state.services.length}> · view · help`, String(state.accentColor || accentColor)),
    paint('35', `└${bar}┘`),
  ];
  ...
  const logEntries = focusedService?.logs?.slice(-10) || [];
  ...
  const logLinesPadded = [...logLines, ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '].slice(0, 10);
  const logViewLines = [
    paint('35', `┌${bar}┐`),
    `│ ${paint(String(state.accentColor || accentColor), clip(`TAROT / ${state.projectName.toUpperCase()}  ·  ${rotatedSigil}  ·  SCRY ${focusedSlot || '—'} / ${focusedService?.label || 'no service'}  ·  ${online}`, Math.max(0, width - 3)))}`,
    paint('35', `├${bar}┤`),
    dockLine(`✦ LIVE CONSOLE  /  ${focusedService?.label || 'No service'}  :${focusedService?.port || '—'}  ·  ${focusedService?.owned ? 'capturing this Tarot session' : 'provider stream unavailable until restarted by Tarot'}`),
    ...logLinesPadded,
    dockLine(`◌ OBSERVE    view = return to Status · scry <1–${state.services.length}> / logs <1–${state.services.length}> = switch stream · help = guide`),
    paint('35', `└${bar}┘`),
  ];
  // A narrow terminal has no room for a boxed dashboard. In compact mode we
  // intentionally remove the purple rails instead of letting them wrap and
  // leave visual debris during a resize.
  const compactServiceLines = state.services.map((service, index) => {
    const live = ['live', 'observed'].includes(service.state);
    const status = service.state === 'error' ? 'ERROR' : live ? 'ONLINE' : service.state === 'starting' ? 'WAKING' : service.state === 'bypassed' ? 'BYPASS' : 'QUIET';
    const color = service.state === 'error' ? '91' : live ? (service.monitorOnly ? '96' : '92') : '2';
    const marker = live ? '●' : service.state === 'error' ? '×' : '○';
    return dockLine(`${marker} ${index + 1} ${service.label} :${service.port} ${status}`, color);
  });
  const compactStatusLines = [
    dockLine(`TAROT / ${state.projectName.toUpperCase()} · ${running}/${total} · ${online}`, String(state.accentColor || accentColor)),
    ...compactServiceLines,
    dockLine(`LINKS: 1 local · 2 computer · 3 LAN`, String(state.accentColor || accentColor)),
    dockLine(`COPY: 1–${state.addresses.length} / C=LAN · scry · help`),
  ];
  const compactHeight = compactStatusLines.length;
  const compactLogEntries = logEntries.slice(-(compactHeight - 2)).map((entry) => {
    const message = typeof entry === 'string' ? entry : entry.message;
    return dockLine(`· ${message}`, typeof entry === 'object' && entry.stream === 'error' ? '91' : '2');
  });
  const compactLogLines = [
    dockLine(`TAROT / ${state.projectName.toUpperCase()} · SCRY ${focusedSlot || '—'} ${focusedService?.label || ''}`, String(state.accentColor || accentColor)),
    dockLine(`logs ${focusedSlot || '—'} · view=status`),
    ...compactLogEntries,
  ];
  while (compactLogLines.length < compactHeight) compactLogLines.push(' ');
  const lines = compact
    ? (state.viewMode === 'logs' ? compactLogLines : compactStatusLines)
    : (state.viewMode === 'logs' ? logViewLines : statusLines);
  const rows = Math.max(terminalRows, lines.length + 2);
  ...
  process.stdout.write(`${restoreCursor ? '\x1b7' : ''}\x1b[?6l\x1b[r${clearViewport ? '\x1b[2J' : ''}\x1b[H`);
...
function setTerminalSize(columns, rows) {
  ...
  if (Number.isFinite(parsedColumns) && parsedColumns >= 4) terminalWidth = parsedColumns;
  if (Number.isFinite(parsedRows) && parsedRows >= 16) terminalRows = parsedRows;
}
...
function precmd() { tarot redraw "$COLUMNS" "$LINES" >/dev/null 2>&1; }
function TRAPWINCH() { tarot reflow "$COLUMNS" "$LINES" >/dev/null 2>&1; zle reset-prompt; }
...
async function launchDock() {
  acquireLock();
  await allocateAdjacentApplicationPorts();
  writeShellConfig();
  await ensureMongo();
  await refresh();

...
async function refresh() {
  await Promise.all([...services.values()].map(async (service) => {
    ...
    // A slow service can finish its database migration after the startup
    // wait expires.  A live, verified listener is authoritative over the
    // transient error we recorded while waiting for it.
    if (service.process && service.verified && await serviceReady(service)) {
      service.state = 'live';
      service.error = '';
    } else if (!service.process) {
      service.state = activeListeners.length ? 'observed' : 'quiet';
      if (service.state === 'observed') service.error = '';
    } else if (service.state === 'error' && service.process.exitCode == null) {
      service.state = 'starting';
      service.error = `Still starting; waiting for ${service.healthPath || '/'} on :${service.port}.`;
    }
  }));
  persist();
}
...
function startupTimeoutMs(service) {
  const configured = Number(service.startupTimeoutMs);
...
async function waitForServiceReady(service, timeoutMs = startupTimeoutMs(service)) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    if ((await listeners(service.port)).length && await serviceReady(service)) return true;
...
async function start(serviceId) {
  ...
  if (service.verified && ['live', 'observed'].includes(service.state)) return `${service.label} is already running under a verified project listener; Tarot did not replace it.`;
  ...
  if (service.process && service.process.exitCode == null) {
    service.state = 'starting';
    service.error = `Still starting; Tarot will continue observing ${service.healthPath || '/'} on :${service.port}.`;
    persist();
    return `${service.label} is still starting on :${service.port}; Tarot will keep observing it.`;
  }
  service.state = 'error';
...
async function startWithDependencies(serviceId, visited = new Set()) {
  ...
  if (visited.has(serviceId)) return [{ serviceId, status: 'blocked', message: `${service.label} has a dependency cycle.` }];
  ...
  for (const dependency of service.dependsOn || []) {
    ...
    if (!['live', 'observed'].includes(dependencyService.state)) {
...
function publicService(service, index) {
  const { process, logs, ...rest } = service;
...
function boundedLogs(reference, requestedLines = 200) {
  ...
  const entries = existsSync(path)
    ? readFileSync(path, 'utf8').split(/\r?\n/).filter(Boolean).slice(-lines).flatMap((line) => {
...
async function handleAgentMessage(message = {}) {
  ...
  if (message.operation === 'action') return fleetAction(message);
...
async function requestControl(parts) {
  const response = await socketRequest({ parts });
...
async function launchAgent() {
  ...
  await refresh();
  heartbeatRegistry([...services.values()].map(({ process, logs, ...service }) => ({ ...service, owned: Boolean(process), logs: logs.slice(-24) })), true);
  agentServer = net.createServer((connection) => {
    let body = '';
    connection.once('data', async (chunk) => {
      body += chunk;
      try {
        const response = await handleAgentMessage(JSON.parse(body));
        connection.end(response.legacy ? response.body : `${JSON.stringify(response)}\n`);
...
async function waitForAgent(timeoutMs = 5000) {
  ...
  while (Date.now() < deadline) {
    try {
      const response = JSON.parse(await socketRequest({ kind: 'fleet', operation: 'snapshot' }));

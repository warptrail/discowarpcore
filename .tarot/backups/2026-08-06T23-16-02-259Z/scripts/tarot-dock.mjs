...
async function probeService(service) {
  ...
  if (!activeListeners.length) return { ok: false, type: contract.type, checkedAt, message: `Nothing is listening on :${service.port}.` };
  if (contract.type === 'tcp' || service.protocol === 'mongodb') return { ok: true, type: 'tcp', checkedAt, message: `TCP listener accepted on :${service.port}.` };
  ...
  try {
    ...
    return { ok, type: 'http', status: response.status, checkedAt, message: ok ? `HTTP ${response.status} from ${path}.` : `HTTP ${response.status} from ${path}.` };
  } catch (error) {
...
async function start(serviceId, options = {}) {
  ...
  service.commandFingerprint = createHash('sha256').update(JSON.stringify({ command: command.command, args: command.args, cwd: command.cwd })).digest('hex');
  ...
  child.once('exit', (code, signal) => {
    ...
    if (!intentional) {
      ...
      if (early && service.restartAttempts < 2) {
        const delays = [1000, 3000];
        const delay = delays[service.restartAttempts];
        service.restartAttempts += 1;
        recordIncident('retry-scheduled', service, `Retry ${service.restartAttempts}/2 scheduled in ${delay} ms.`, { runId: service.runId });
        setTimeout(() => enqueueAction(() => start(service.id, { retry: true })).catch(() => {}), delay);
      }
    }
    persist();
  });

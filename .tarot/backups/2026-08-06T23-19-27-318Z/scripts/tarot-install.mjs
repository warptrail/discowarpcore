...
async function install() {
  ...
  const payload = [...release.files, 'tarot-port.release.json'].map((file) => ({ file, source: resolve(source, file), destination: resolve(targetRoot, file) }));
  ...
  if (targetPackage) {
    targetPackage.scripts = {
      ...
      'tarot:doctor': 'node scripts/tarot-dock.mjs doctor',
      'tarot:report': 'node scripts/tarot-dock.mjs report',
    };

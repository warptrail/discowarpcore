...
function atomicJson(path, value) {
  const temporary = `${path}.tarot-update-${process.pid}.tmp`;
  writeFileSync(temporary, `${JSON.stringify(value, null, 2)}\n`);
  renameSync(temporary, path);
}

function restoreTransaction(targetRoot, backupRoot, records) {
  for (const record of records) {
    if (record.existed) {
      const backup = resolve(backupRoot, record.relative);
      mkdirSync(dirname(record.destination), { recursive: true });
      cpSync(backup, record.destination);
    } else {
      try { rmSync(record.destination, { force: true }); } catch (error) {
...
async function syncRegistry(manifest, targetRoot, release) {
  ...
  try {
    ...
    return `Registry metadata synchronized. Existing ports were preserved; use tarot registry reconcile --apply for an intentional port move.`;
  } catch (error) {
...
if (!existsSync(packagePath)) {
  ...
  if (!existsSync(releasePath)) {
    ...
    for (const file of release.files || []) {
      ...
      if (!existsSync(source)) throw new Error(`Master payload is missing ${file}.`);
      if (release.hashes?.[file] && fullDigest(source) !== release.hashes[file]) throw new Error(`Master payload hash mismatch for ${file}.`);
    }
    ...
    const planned = [...release.files, 'tarot-port.release.json'].map((file) => {
      const source = resolve(sourceRoot, file);
    ...
    const agentScript = 'node scripts/tarot-agent.mjs';
    ...
    const doctorScript = 'node scripts/tarot-dock.mjs doctor';
    const reportScript = 'node scripts/tarot-dock.mjs report';
    ...
    const scriptsCurrent = !scriptConflict && targetPackage.scripts?.tarot === tarotScript && targetPackage.scripts?.['tarot:agent'] === agentScript && targetPackage.scripts?.['tarot:update'] === updaterScript && targetPackage.scripts?.['tarot:storm'] === stormScript && targetPackage.scripts?.['tarot:install'] === installerScript && targetPackage.scripts?.['tarot:doctor'] === doctorScript && targetPackage.scripts?.['tarot:report'] === reportScript;
    ...
    if (verifyOnly) {
      ...
      console.log('\nAlready current; no runtime files were written.');
      if (targetRoot !== sourceRoot) {
        const registryResult = await syncRegistry(targetManifest, targetRoot, release);
        atomicJson(manifestPath, targetManifest);
      ...
      const stageRoot = resolve(targetRoot, '.tarot', 'update-staging', stamp);
      mkdirSync(backupRoot, { recursive: true });
      targetManifest.version = release.version;
      targetManifest.tarotVersion = release.version;
      targetManifest.manifestVersion = 1;
      ...
      targetPackage.scripts = { ...(targetPackage.scripts || {}), 'tarot:agent': agentScript, 'tarot:update': updaterScript, 'tarot:storm': stormScript, 'tarot:install': installerScript, 'tarot:doctor': doctorScript, 'tarot:report': reportScript };
      if (!scriptConflict || forceScript) targetPackage.scripts.tarot = tarotScript;
      const records = [manifestPath, packagePath, ...planned.map((item) => item.destination)].map((destination) => ({
        destination,
        relative: relative(targetRoot, destination),
      ...
      for (const record of records) {
        ...
        cpSync(record.destination, backup);
      }
      atomicJson(resolve(backupRoot, 'restoration-manifest.json'), { schemaVersion: 1, createdAt: new Date().toISOString(), targetRoot, records });
      try {
        for (const item of planned) {
          const staged = resolve(stageRoot, item.file);
          mkdirSync(dirname(staged), { recursive: true });
          cpSync(item.source, staged);
          if (release.hashes?.[item.file] && fullDigest(staged) !== release.hashes[item.file]) throw new Error(`Staged hash mismatch for ${item.file}.`);
        }
        atomicJson(resolve(stageRoot, 'tarot.manifest.json'), targetManifest);
        atomicJson(resolve(stageRoot, 'package.json'), targetPackage);
        for (const item of planned) {
          mkdirSync(dirname(item.destination), { recursive: true });
          renameSync(resolve(stageRoot, item.file), item.destination);
        }
        renameSync(resolve(stageRoot, 'tarot.manifest.json'), manifestPath);
        renameSync(resolve(stageRoot, 'package.json'), packagePath);
        const verified = planned.every((item) => existsSync(item.destination) && fullDigest(item.source) === fullDigest(item.destination));
        if (!verified) throw new Error('Post-update checksum verification failed.');
        ...
        console.log(registryResult);
        console.log('If an older agent is still running, use farewell and reopen Tarot when you are ready to activate 1.9.0.');
        ...
        restoreTransaction(targetRoot, backupRoot, records);
        fail(`Update rolled back automatically: ${error.message}. Restoration manifest: ${resolve(backupRoot, 'restoration-manifest.json')}`);
      } finally {
